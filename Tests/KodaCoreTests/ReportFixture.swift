...
enum ReportFixture {
    ...
    struct Page {
        ...
        var hasFacts = true
        var title: String? = "A perfectly reasonable page title here"  // 37 chars
        var titleCount = 1
        var desc: String? = String(repeating: "d", count: 100)
        var descCount = 1
        var h1: String? = "A heading"
        var h1Count = 1
        var h2Count = 2
    ...
    static let pages: [Page] = [
        ...
        Page(path: "/title-is-h1", title: "Identical on both", h1: "Identical on both"),

        // Meta description
        Page(path: "/no-desc", desc: nil, descCount: 0),
        Page(path: "/dupe-desc-a", desc: "Shared description text"),
        Page(path: "/dupe-desc-b", desc: "Shared description text"),
            ...
            for page in pages + external + images {
                ...
                try db.execute(
                    sql: """
                        ...
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'en',400,?)
                        """,
                    arguments: [id, page.title, page.title?.count, page.titleCount,
                                page.desc, page.desc?.count, page.descCount,
                                page.h1, page.h1Count, page.h2Count, canonical,
